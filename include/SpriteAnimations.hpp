...
struct AnimFrame {
	std::string subID; // The subTexture id being used. (Builds off of Spritesheet)
	SDL_RendererFlip flip;
	int durationMs;

	AnimFrame(const std::string& subTextureID, int durationMs, SDL_RendererFlip flip = SDL_FLIP_NONE)
		: subID(subTextureID), durationMs(durationMs), flip(flip) {
	}
...
class AnimatedSprite{
	...
	void addSequence(const std::shared_ptr<Spritesheet>& sprite, const std::vector<std::pair<std::string, int>>& frames, bool loop = true);
};
